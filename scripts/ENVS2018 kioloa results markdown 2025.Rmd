...
* Sedge, fern and tree cover were collected at some but not all sites, and are included in 'Other' for the overall percent cover ranking. 
* Sedge and fern were high value cover types in some sites and have been highlighted, but cannot be directly compared with other sites because not reported does not mean they weren't there. 
...
biometric_table <- gt(biometric_data) %>%
  ...
  tab_style(style = list(cell_fill(color = "#2A788E33")),
    locations = cells_body(rows = 7:9, columns = 1:2)) %>%
  tab_style(style = list(cell_fill(color = "#22A88433")),
    locations = cells_body(rows = 10:15, columns = 1:2)) %>%
  tab_style(style = list(cell_fill(color = "#7AD15133")),
    locations = cells_body(rows = 16:19, columns = 1:2)) %>%
  tab_style(style = list(cell_fill(color = "#FDE72533")),
    locations = cells_body(rows = 20:23, columns = 1:2)) %>%
...
ggplot(count) +
  ...
  facet_wrap(~feature) +
  theme_classic()+
  # reanme primary axis and add secondary axis for CWD
  scale_y_continuous(name = "Count of trees / length of CWD (m)")
```
...
Interesting! The upper slope forest had the highest species richness, with an average of 13 species. The paddock (unsuprisingly) had the lowest. 

Note, the rainforest and the coastal forest sites were only surveyed once - which (as we know from the above section on beta diversity) will artificially deflate their overall richness compared to the sites that were visited multiple times. That means, with additional visits, the rainforest and coastal forest diversity may have been higher. Indeed, in 2024 the rainforest had the highest diversity (mean 14 species).

There was a significant difference in richness between the community types (p = 0.006). Note: this is only testing the difference in the number of species, not the difference in their diversity.

The Tukey test plot shows us the direction and strength of the pairwise differences; e.g. in the 'LSF-P' pairing, lower slope forest has many more species than paddock, and since this crosses the zero, this is a significant difference. 
...
There was no significant in Simpson's diversity index among the communities.  
...
The major cluster in the middle are the forest birds. We can break this big cluster down further into subset clusters therein. Notably:
...
For example, the Masked Lapwing heard on the acoustic meter might have been further away than 50m because their call carries. We could have tested this by playing a known volume tone at 50m from the meters to figure out how 'loud' something needs to be to be in the site.
...
We detected more species on the expert counts in the casuarina forest, lower and upper slope forests. There was no effect of sampling type on species richness (i.e. no significant difference between the methods for determining species richness).
...
However we've only looked at species richness. How about the beta diversity between the acoustic and expert counts?
...
There was no significant difference between the survey methods in the diversity of species detected.
...
We recorded two new species for ENVS2018 in 2024: spotted grass frog (*Limnodynastes tasmaniensis*). 
